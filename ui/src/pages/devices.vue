...
import DevicesVue from "@/components/grids/Devices.vue"
  ...
  const paramToOptions = ((params: Array<any>, labelSwitch: number) => {
    ...
    options.splice(0,0, { value: null, label: "Any" })
    return options
...
<template>
  <card :icon="autoUpdate ? 'MonitorCellphone' : 'Pause'" :headingTitle="filterdTitle" has-table class="mb-6" headerIcon="PlusBox" @header-icon-click="add" headerIconTitle="Add device">
    ...
    <template v-slot:filter>
      <field inline label="Status"><control v-model="filters.statusID" :options="paramToOptions(Statuses, 0)" /></field>
      <field inline label="Maintainer"><control v-model="filters.maintainerID" :options="paramToOptions(Maintainers, 0)" /></field>
      <field inline label="Location"><control v-model="filters.locationID" :options="paramToOptions(Locations, 0)" /></field>
      <field inline label="Category"><control v-model="filters.categoryID" :options="paramToOptions(Categories, 0)" /></field>
      <field inline label="Type"><control v-model="filters.deviceTypeID" :options="paramToOptions(DeviceTypes, 0)" /></field>
      <field inline label="Connection"><control v-model="filters.interfaceTypeID" :options="paramToOptions(InterfaceTypes, 0)" /></field>
      <field inline label="Last Seen"><control v-model="filters.lastseenID" :options="lastSeenOptions" /></field>
