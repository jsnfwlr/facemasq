...
import { useDevices, Device, Netface, Address, Hostname, Connection, ColumnSort } from '@/stores/devices'
...
const emit = defineEmits(["edit", "discard", "save", "delete", "add"])
...
const allAddresses = (device: Device) => {
  const records = <Array<MultiRecord>>[]
...
const columnClass = (column: string, param1: any, param2: any) => {
  let colClass = [column]
  switch (column) {
    case "ip":
      if (param1 == 1) {
...
const colCount = (excludeCount: number) => {
  return (maxCols[props.mode] - excludeCount)
}
...
const expandedRows = ref<{Devices: Array<number>, Notes: Array<number>}>({ Devices: [], Notes: [] })
...
const toggleRowExpand = (deviceIndex: number) => {
  if (isRowExpanded(deviceIndex)) {
    ...
    deviceStore.$patch((state) => { state.focusedItems.devices.delete(deviceIndex.toString()) })
  } else {
    expandRow(deviceIndex)
    deviceStore.$patch((state) => { state.focusedItems.devices.set(deviceIndex.toString(), clonedeep(props.items[deviceIndex])) })
  }
}
...
const edit = (deviceIndex: number) => {
  if (isEditing(deviceIndex)) {
    deviceStore.$patch((state) => { state.editingItems.devices.delete(deviceIndex.toString()) })
  } else { 
    deviceStore.$patch((state) => { state.editingItems.devices.set(deviceIndex.toString(), clonedeep(props.items[deviceIndex])) })
    collapseRow(deviceIndex)
  }
}
...
<template>
  <div class="devices datagrid">
    <modal-box v-model="isModalActive" title="edit.Label" >
      <div v-for="(address, index) in investigations.get(investigateID)">
        {{ address.AddressID }}
    ...
    <table :class="props.mode" class="multibody cardify">
      ...
      <tbody v-for="(row, index) in items" :key="row.Interfaces[0].Addresses[0].ID">
        <tr :class="rowClass(index)">
          ...
          <td :data-label="row.Label" class="whitespace-nowrap name"><div><mdIcon :icon="getLabelByID(DeviceTypes, 'Icon', row.DeviceTypeID)" size="14" />{{ row.MachineName }}</div></td>
          <td data-label="Maintainer" class="maintainer"><div>{{ rowClass(index).includes("invading") ? "-" :getLabelByID(Maintainers, "Label", row.MaintainerID) }}</div></td>
          <td data-label="Location" class="location"><div>{{ rowClass(index).includes("invading") ? "-" : getLabelByID(Locations, "Label", row.LocationID) }}</div></td>
          <td data-label="Description" class="description"><div class="text-gray-500 dark:text-gray-400 whitespace-nowrap">{{ row.Label }}</div></td>
          <td data-label="IP" class="ip"><div :class="columnClass('ip', row.Primary.IsReservedIP, null)">{{ row.Primary.IPv4 }}</div></td>
          <td data-label="MAC" class="mac"><div><mdIcon :icon="iconIFace(row.Primary.InterfaceTypeID)" size="10" />{{ row.Primary.MAC }}</div></td>
          <td data-label="First Seen" class="firstseen"><div>{{ fuzzyTime(row.FirstSeen) }}</div></td>
          <td data-label="Last Seen" class="lastseen"><div><mdIcon :title="getLabelByID(Statuses, 'Label', row.StatusID)" :icon="getLabelByID(Statuses, 'Icon', row.StatusID)" size="14" /> {{ fuzzyTime(row.Interfaces[0].LastSeen) }}</div></td>
          <td data-label="Operating System" class="os"><div>{{ rowClass(index).includes("invading") ? "-" : getLabelByID(OperatingSystems, "Family", row.OperatingSystemID) + " " + getLabelByID(OperatingSystems, "Version", row.OperatingSystemID) }}</div></td>
          <td data-label="Make+Model" class="brand"><div>{{ (rowClass(index).includes("invading") && row.Brand === null) ? "" : ((row.Brand) ? row.Brand : "?") + " " + ((row.Model) ? row.Model : "?") }}</div></td>
          <td data-label="Connectivity" class="connectivity"><div><connectivity-indicator :includeDate="false" :data="row.Interfaces[0].Addresses[0].Connectivity" /></div></td>
        ...
        <tr v-show="isRowExpanded(index)" v-if="allAddresses(row).length > 0" v-for="record in allAddresses(row)" :class="rowClass(index)" class="address-expansion">
          <td class="expand"/>
          <td class="name" />
          <td class="maintainer" />
          <td class="location" />
          <td class="description" />
          <td class="ip"><div :class="columnClass('ip', record.IsReservedIPv4, null)">{{ record.IPv4 }}</div></td>
          <td class="mac"><div><mdIcon :icon="iconIFace(record.InterfaceTypeID)" size="10" />{{ record.MAC }}</div></td>
          <td class="firstseen" />
          <td class="lastseen"><div><mdIcon :title="getLabelByID(Statuses, 'Label', record.StatusID)" :icon="getLabelByID(Statuses, 'Icon', record.StatusID)" size="14" />{{ fuzzyTime(record.LastSeen) }}</div></td>
          <td class="os" />
          <td class="brand" />
          <td class="connectivity"><div><connectivity-indicator :includeDate="false" :data="record.Connectivity" /></div></td>
          <td v-if="userStore.hasAccess('details', 'read')" class="actions right" >
    ...
    & > table {
      & > thead, & > tbody {
        & > tr {
          & > td, & > th {
            & > & { @apply hidden; }
            &.expand { @apply lg:table-cell lg:w-[16px]; }
            &.name { @apply flex lg:table-cell lg:w-[140px]; }
            &.description { @apply flex lg:table-cell lg:w-[230px]; }
            &.brand { @apply lg:table-cell lg:w-[160px] lg:text-center; }
            &.os { @apply lg:table-cell lg:w-[160px] lg:text-center; }
            &.serial { @apply lg:table-cell lg:w-[160px]; }
            &.connectivity { @apply flex lg:table-cell lg:w-[200px]; }
            &.location { @apply lg:table-cell lg:w-[140px] lg:text-center; }
            &.maintainer { @apply lg:table-cell lg:w-[140px] lg:text-center; }
            &.mac { @apply flex lg:table-cell lg:w-[160px]; }
            &.ip { @apply flex lg:table-cell lg:w-[160px]; }

            &.firstseen { @apply flex lg:table-cell lg:w-[160px]; }
            &.lastseen { @apply flex lg:table-cell lg:w-[160px];  }
            &.actions { @apply flex lg:table-cell lg:w-[160px] text-center lg:text-right;  }
            &.actions > div { @apply w-full lg:w-auto; }
          }
        }
      }
      &.cardify {
