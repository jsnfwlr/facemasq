...
func (record *Model) Save() (err error) {
	...
	if record.ID == 0 {
		sql := `INSERT INTO ` + TABLENAME + ` (Label, BitSpace, Notes) VALUES (?,?,?);`
		result, err = db.Conn.Exec(sql, record.Label, record.BitSpace, record.Notes)
		if err != nil {
			return
		}
		record.ID, err = result.LastInsertId()
	} else {
		sql := `UPDATE ` + TABLENAME + ` SET Label = ?, BitSpace = ?, Notes = ? WHERE ID = ?;`
		_, err = db.Conn.Exec(sql, record.Label, record.BitSpace, record.Notes, record.ID)
	}
	return
}
