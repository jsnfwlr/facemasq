			...
			if len(addresses) > 0 {
				...
				go func(iface net.Interface) {
					...
					if err := ScanARP(&iface); err != nil {
						logging.Error("interface %v: %v", iface.Name, err)
					}
				}(netFace)
			}
		}
...
func ScanARP(iface *net.Interface) error {
	...
	for {
		...
		if err := writeARP(handle, iface, addr); err != nil {
			logging.Error("error writing packets on %v: %v", iface.Name, err)
			return err
		}
		...
		case packet = <-in:
			...
			if err != nil {
				if err.Error() == "sql: no rows in result set" {
					...
					if err != nil {
						logging.Error("api/lib/arpscan/arpscan.go:177 Err: %v\n", err)
					}
					scanID, _ = sqlres.LastInsertId()
				} else {
					logging.Error("api/lib/arpscan/arpscan.go:181 Err: %v\n", err)
				}
			}
