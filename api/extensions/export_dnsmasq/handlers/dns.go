...
func WriteDNSConfig(out http.ResponseWriter, in *http.Request) {
	...
	if err != nil {
		logging.Error("Error getting Hostname Records: %v", err)
	...
	if err != nil {
		logging.Error("Error getting Hostname suffixes: %v", err)
	...
	if err != nil {
		logging.Error("Error parsing Hostname suffixes: %v", err)
