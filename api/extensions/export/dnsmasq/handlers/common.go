...
const templateName = "header.tmpl"
...
func getTemplateFilePath() (tmplPath string, err error) {
	...
	templateDir, err = files.GetDir(fmt.Sprintf("%[2]s%[1]c%[3]s%[1]c%[4]s", os.PathSeparator, "templates", "export", "dnsmasq"))
	if err != nil {
		return
	}
...
func getFileHeaderFromTemplate() (fileHeader string, err error) {
	...
	if err != nil {
		logging.Error("Unable to determine template folder location: %v", err)
		return
	}
	...
	if err != nil {
		logging.Error("error executing template : %v", err)
		return
	}
