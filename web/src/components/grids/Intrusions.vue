<script setup lang="ts">
  ...
  const { editingItems, deletingItems, focusedItems, dosiers } = storeToRefs(deviceStore)
  ...
  const dosierID = ref(0)

  const viewDosier = (id: number) => {
    dosierID.value = id
    isModalActive.value = true
    deviceStore.getDosier(id)
    console.log("Show the user the dosier for " + id)
    // this.$router.push('/dosier/' + id)
  }
...
<template>
  <div class="devices datagrid">
    <modal-box v-model="isModalActive" title="edit.Label">
      <div v-for="(address, index) in dosiers.get(dosierID)" :key="'dosier-id:' + index">
        {{ address.AddressID }}
        ...
        <tr :class="rowClass(index)" @click="toggleRowFocus(index)">
          ...
          <td class="actions right" v-if="userStore.hasAccess('devices', 'write')">
            <div class="flex flex-end">
              <btn v-if="rowClass(index).includes('invading') && userStore.hasAccess('details', 'read')" color="danger" title="View Dosier" icon="Magnify" :outline="true" small @click="viewDosier(index)" />
