...
RUN mkdir -p /build/api /build/i18n /dist/api /dist/logs /dist/extensions /deps
RUN <<EOT
...
RUN --mount=type=cache,target=/root/.cache --mount=type=cache,target=/go/pkg/mod find ./extensions -maxdepth 1 -type d | grep "\/.*\/" | xargs -n 1 -I {} bash -c 'CGO_ENABLED=1 goxx-go build --tags "linux" -buildmode=plugin -o /dist/{}.so {}'
RUN cp /lib/$(uname -i)-linux-gnu/libdl-*.so /dist/libdl.so
RUN cp /lib/$(uname -i)-linux-gnu/libdl.so.2 /dist/libdl.so.2
